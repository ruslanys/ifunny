...
в ходе индексации могут и будут встречаться ошибки. Не потому, что парсер может быть плохо написан (хотя и по этой причине),
...
Как только появляется полностью знакомая страница, текущая позиция в ленте (номер Страница) очищается и индексация прекращается.
